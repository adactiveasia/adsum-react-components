...
class ClientAPI {
  ...
  async init(config, dir = "/local") {
    this.entityManager = new EntityManager(
      Object.assign({}, config, { cacheManager: new DistCacheManager(dir) })
    );
  }

  async load(cacheFirst = true, allowOutdatedCache = true) {
    if (cacheFirst) {
      return this.entityManager.loadFromCache(allowOutdatedCache).catch(() => {
  ...
  getPoi(id) {
    ...
    if (poi && poi.logos && poi.logos.values) {
      for (const logo in poi.logos.values) {
        if (
          poi.logos.values[logo] &&
          poi.logos.values[logo].value &&
          !Number.isNaN(poi.logos.values[logo].value)
        ) {
    ...
    return pois.map((poi: Object): Object => {
      if (!poi.logo)
        poi.logo =
          ...
          poi.logos.values &&
          poi.logos.values[0] &&
          poi.logos.values[0].value
            ? this.getFile(poi.logos.values[0].value)
            : null;
      return poi;
    });
  }
  ...
  getMediasByPlaylistTag(tagName: string): Array<Object> {
    ...
    if (playlist.length) {
      medias = this.entityManager
        .getRepository("Media")
        .getList(playlist[0].medias.toJSON());
    ...
    if (this._allCategories === null) {
      ...
      this._allCategories = categories.map((category) => {
        if (
          category.logo &&
          category.logo.value &&
          !Number.isNaN(category.logo.value)
        ) {
        ...
        if (poi.logos && poi.logos.values) {
          for (const logo in poi.logos.values) {
            if (
              ...
              poi.logos.values[logo] = this.getFile(
                poi.logos.values[logo].value
              );
            }
          }
